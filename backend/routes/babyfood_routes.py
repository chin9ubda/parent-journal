...
def create_babyfood(body: BabyfoodBody, user: dict = Depends(get_current_user)):
    ...
    with get_db() as conn:
        c = conn.cursor()
        c.execute(
            '''INSERT INTO babyfood_records(user_id, date, ingredient, reaction, memo, created_at)
               VALUES(?,?,?,?,?,?)''',
            (uid, body.date, body.ingredient, body.reaction, body.memo, now)
        )
        rid = c.lastrowid
        conn.commit()
...
@router.get('/babyfood')
def list_babyfood(user: dict = Depends(get_current_user)):
    uid = user['uid']
    with get_db() as conn:
        c = conn.cursor()
        c.execute(
            '''SELECT id, date, ingredient, reaction, memo
